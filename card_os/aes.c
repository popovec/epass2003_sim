...
static void addEKey(uint8_t * state, uint8_t * key)
{
	uint8_t i;
	for (i = 0; i < 16; i++)
		state[i] ^= key[i];
...
*/
static void ek(uint8_t * key, uint8_t keysize, uint8_t * ta)
{
	uint8_t rc = 1, j;
	uint8_t *k = KEY;
	uint8_t i, l;
...
static uint8_t sbox_helper(uint8_t r)
{
	uint8_t tmp;
...
static uint8_t aes_init(uint8_t * key, uint8_t keysize, uint8_t * ta)
{
	uint8_t i, t, tt;
