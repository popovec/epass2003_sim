/*
    ...
    Note: epass2003 uses only secp256r1/nistp256/prime256v1 curve.
*/
...
/*
    ...
    multiplication.  3 or 4 bytes of random data are recommended.  If speed
...
uint8_t mp_get_len(void)
{
	return mod_len;
}

static void mp_set_len(uint8_t a)
{
	mod_len = a;
}
...
*                       modular arithmetic                               *
***************************************************************************/
static void mul_mod(bignum_t * result, bignum_t * a, bignum_t * b, bignum_t * mod);
/**************************************************************************
...
uint8_t __attribute__((weak)) mp_sub(bignum_t * r, bignum_t * a, bignum_t * b)
{
	return bn_sub(r, a, b);
}
...
void field_add(bignum_t * r, bignum_t * a)
{
	add_mod(r, a, field_prime);
}

static void field_sub(bignum_t * r, bignum_t * a)
{
	sub_mod(r, a, field_prime);
}

static void mul_mod(bignum_t * c, bignum_t * a, bignum_t * b, bignum_t * mod)
{
	DPRINT("%s\n", __FUNCTION__);
...
*/
static void secp256k1reduction(bignum_t * result, bigbignum_t * bn)
{
	bignum_t *h;
...
// Create A23,A23 for D3 overwrite A22 by A23 in operand
	...
	ptr[12 * 4] = mp_add((bignum_t *) (ptr + 10 * 4), (bignum_t *) (ptr + 22 * 4));
// T: A23||A22||A21||A20||0
	memset(ptr + 7 * 4, 0, 4);
...
result=t+s2+s3+s4 -d1
 ...
 S3 > D1, there is enough to calculate with 16 bytes
...
reduce r mod p256 by subtraction up to 10 multiples of p256
...
static void fast256reduction(bignum_t * result, bigbignum_t * bn)
{
...
// use T as TMP
	...
	*(ptr_l + 7 * 4) = mp_shiftl((bignum_t *) (ptr_l + 3 * 4));
	// TMP += S3
	mp_set_len(8);
	carry += mp_add((bignum_t *) (ptr_l + 6 * 4), (bignum_t *) (ptr_l + 14 * 4));
...
#if MP_BYTES >= 66
static void fast521reduction(bignum_t * result, bigbignum_t * bn)
{
...
static uint8_t ec_is_point_affine(ec_point_t * p, struct ec_param *ec)
{
	bignum_t yy;
	bignum_t xx;
	bignum_t xxx;
...
static void ec_point_1_1_0(ec_point_t * a)
{
...
static void ec_full_add(ec_point_t * result, ec_point_t * s, ec_point_t * t)
{
...
static void ec_mul(ec_point_t * point, uint8_t * k)
{
	int8_t i;
	uint8_t b, b2, j;
	uint8_t index;
...
a=192 b=6  mask=40
*/
static void ec_set_param(struct ec_param *ec)
{
...
#endif
static void
    __attribute__((noinline)) ec_blind_key(uint8_t * blind_key, bignum_t * order)
{
...
// point = k * point
static uint8_t ec_calc_key(bignum_t * k, ec_point_t * point, struct ec_param *ec)
{
