...
{
	DPRINT("return_status   r->len16 = %d\n", iso_response.len16);
...
// return response ADPU and status to reader
	if ((iso_response.orig_cla & 0x0c) > 4) {
		ret = sm_wrap_response(&iso_response, message, ret);
		DPRINT("SM wrap returned %d bytes\n", ret);
	}
#ifdef T1_TRANSPORT
	if (iso_response.protocol == 0)
...
// TODO if P1 == 2 challenge for authentyfication ..
	if (M_P1 != 0 || M_P2 != 0)
		return S0x6a86;	//incorrect P1,P2
...
static uint8_t iso7816_read_record(uint8_t * message, struct iso7816_response *r)
{
	DPRINT("%s P1: %02x P2: %02x LC: %02x\n", __FUNCTION__, M_P1, M_P2, M_P3);
	if (M_P2 != 4) {
		DPRINT("Warning, %s is working only for P2 = 4\n", __FUNCTION__);
		return S0x6a83;	// record not found
	}
	return fs_read_record(M_P1, r);
}

#if 0
static uint8_t iso7816_write_record(uint8_t * message, struct iso7816_response *r)
{
	uint8_t record;
	...
	if ((M_P2 & 7) > 4)
		return S0x6a86;	// incorrect parameter P1,P2
	if (M_P2 & 0xf8)
		return S0x6a81;	// Function not supported (see table 47 in ISO 7816-4, short ientifier...
	// P2 first, last, next previous or P1, (card_os/fs.h, ISO7816-4 table 52)
	return fs_write_record(M_P1, M_P2, r);
}
#endif
static uint8_t iso7816_append_record(uint8_t * message, struct iso7816_response *r)
{
	DPRINT("%s P1: %02x P2: %02x LC: %02x\n", __FUNCTION__, M_P1, M_P2, M_P3);
	...
	return fs_write_record(0, T_RECORD_APPEND, r);
}
...
//
// delete file, epass2003 violates ISO7816-9:2017(E):
// Table 3:
// for encoding P1=0, P2=0 the command data field is empty.
// Table 5: 
// Ne is always 0.
//
// epass2003 uses APDU "00 e4 00 00 02 29 00" to delete file 2900
// correct APDU is:    "00 e4 00 02 02 29 00" (delete EF under current DF)
//
// error codes:
...
//
// if selected file is deleted, the parent becomes the current file
// if a file is delete is not current file, old current file may remain same
// if DF is deleted, complete sub tree is deleted 
...
static struct f_table cla00[] = {
	...
	{ATTR_T0_Le_present, 0x46, epass2003_generate_key},	// iso7816-8
	{0, 0x82, epass2003_external_key_auth},
	...
	{APDU_Ne | APDU_Lc_empty | ATTR_T0_P3NE, 0xb2, iso7816_read_record},	// iso7816-4/7.3.3 Nc = 0, data field absent, P3 is Le
	{APDU_Ne | APDU_Lc_empty | ATTR_T0_P3NE, 0xc0, iso7816_get_response},	// iso7816-4/7.6.2 Nc = 0
	//{0, 0xc2, iso7816_envelope},
	{APDU_Ne | APDU_Lc_empty | ATTR_T0_P3NE, 0xca, epass2003_get_data},	// iso7816-4/7.4.2 Nc = 0, Nc > 0 for 0xcb...
	...
	{APDU_Nc | APDU_Le_empty, 0xe2, iso7816_append_record},
...
static struct f_table cla80[] = {
	...
	{APDU_Lc_empty | APDU_Ne, 0x34, epass2003_list_files},
	{APDU_Nc | APDU_Le_empty, 0xda, w_fs_key_change_type},	// proprietary ..
	{APDU_Nc, 0x50, epass2003_mutual_auth},	// case 3
	{APDU_Nc, 0xb4, epass2003_read_generated_public_key},	// case 4
	{APDU_Nc, 0xe3, epass2003_install_secret_key},
	{APDU_Nc | APDU_LONG, 0xe7, epass2003_import_key},
	{0xff}
};
...
static uint8_t parse_apdu(uint16_t input_len)
{
	...
	// unwrap APDU if SM is used
	iso_response.orig_cla = cla;
	if (cla & 0x0c) {
		uint8_t sm_ret;

		sm_ret = sm_unwrap(r, input_len);
		if (S_SM_OK != sm_ret)
			return sm_ret;
		DPRINT("SM unwrapped..\n");
