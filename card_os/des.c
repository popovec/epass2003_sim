/*
    ...
    In 2023, the 2des implementation was added.
*/
#include <string.h>
#include <stdint.h>
#include "des.h"
...
static void rotate_key_r(uint8_t * key)
{
	uint8_t carry, tmp;
	int8_t i;
	carry = 0;
...
static uint8_t permute1(TAB_TYPE ** table, uint8_t * from)
{
	uint8_t i;
	uint8_t shift_size;
	uint8_t byte;
...
static void permute(uint8_t count, TAB_TYPE * table, uint8_t * from, uint8_t * to)
{
	while (count--)
...
static void des_key_perm(uint8_t * from, uint8_t * to)
{
	uint8_t i, j;
	uint8_t tmp, tmp0 = 0;
...
static void des_iip(uint8_t * from, uint8_t * to)
...
static void des_ip(uint8_t * from, uint8_t * to)	//  initial permutation
{
	uint8_t i, j;
...
#ifdef DES3
// correction of mask ...
	if (mode == (DES_3DES | DES_DECRYPTION_MODE))
		mode = 0x93;
	mode += 0x4a;
// 3DES EDE mode decipher
	if (mode & 16)
		main_key += 16;
