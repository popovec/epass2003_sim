/*
    ...
    Copyright (C) 2019-2023 Peter Popovec, popovec.peter@gmail.com
...
/*
 ...
 env variable EPASS2003_DEBUG is used to control debug messages

 variable is bit mapped, bits 0,1,2,3 are reserved for simulated reader and pcscd
 
  1 - run pcscd with -d switch
  2 - enable debug from EPASS2003 simulated reader (ifHandler)
  4 - RFU
  8 - epass2003
...
#include <stdlib.h>
#include <stdio.h>
#define DPRINT(msg...) {char *env_atr = getenv ("EPASS2003_DEBUG"); if(env_atr){if(atoi(env_atr)& DEBUG_V){fprintf(stderr,msg);}}}
...
static void __attribute__((unused)) HPRINT(char *m, void *d, int size)
{
	...
	char *env_atr = getenv("EPASS2003_DEBUG");
	if (env_atr) {
