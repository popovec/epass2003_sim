...
- RSA operation is running in constant time, fixed size window (2,4 or 5
  ...
  is compiled with AVR ASM routines for BN arithmetic)

- Exponent is blinded (24 bit of random data per exponentiation)

- CRT calculation is protected against Bellcore attack (single error attack)
...
static uint8_t rsa_get_len(void)
{
	return mod_len;
}

static void rsa_set_len(uint8_t len)
{
	mod_len = len;
}
...
uint8_t __attribute__((weak)) mp_add_v(uint8_t * r, uint8_t * a, uint8_t len, uint8_t carry)
{
	return bn_add_v(r, a, len, carry);
}

uint8_t __attribute__((weak)) mp_sub_v(uint8_t * r, uint8_t * a, uint8_t * b, uint8_t len)
{
...
uint8_t __attribute__((weak)) rsa_inv_mod(rsa_num * result, rsa_num * a, rsa_num * mod)
{
...
static void rsa_square(rsa_long_num * r, rsa_num * a)
{
#if RSA_BYTES == 64
	if (rsa_get_len() == 32)
...
//static void
void rsa_mul(rsa_long_num * r, rsa_num * a, rsa_num * b)
{
#if RSA_BYTES == 64
	if (rsa_get_len() == 32)
...
void rsa_mul_mod_half(rsa_half_num * r, rsa_half_num * a, rsa_half_num * b)
{
#if RSA_BYTES == 64
	if (rsa_get_len() == 32)
...
void rsa_mul_half(rsa_num * r, rsa_half_num * a, rsa_half_num * b)
{
	if (rsa_get_len() == 32)
...
void partial_barret(rsa_long_num * num, rsa_num * Bc)
{
	...
	rsa_mul_half(&tmp, (rsa_half_num *) Bc, (rsa_half_num *) (offset + (uint8_t *) num));
...
// Here is modified extended euclidean algo for calculation r - n^ -1 mod r.
...
uint8_t __attribute__((weak))
    monPro0(rsa_long_num * t, rsa_long_num * help1, rsa_num * n, rsa_half_num * Mc, rsa_num * Bc)
{
	uint8_t carry;
	...
	rsa_mul_half((rsa_num *) help1,
		     (rsa_half_num *) Bc, (rsa_half_num *) (offset + (uint8_t *) t));
	...
	rsa_mul_half((rsa_num *) help1, mm, (rsa_half_num *) (hsize + (uint8_t *) n));
...
static uint8_t
monPro_square(rsa_long_num * t, rsa_long_num * tmp, rsa_num * n, rsa_half_num * Mc, rsa_num * Bc)
{
...
//                 tmp    result1        result2/A
static uint8_t
monPro_1(rsa_long_num * t, rsa_long_num * tmp, rsa_num * n, rsa_half_num * Mc, rsa_num * Bc)
{
...
////////////////////////////////////////////////////
// montgomery exponentiation (for maximum 255*8 bits!)
#if RSA_BYTES > 255
#error Please check i variable
#endif

/* "best" E_bits for RSA:               numbers of multiplications/ram
Key length      CRT exponentiation	Ebits 4	   Ebits 5  Ebits 6
...
#if E_BITS == 5
static uint8_t get_bits5(rsa_exp_num * exp, uint16_t count)
{
	uint8_t byte, bit;
	uint16_t sample;
...
/* x_ is original input number to exponentiate (not in Montgomery format)
   in x_ exponentiation result is returned
...
// (load M_[x] into t[1]) but code is then bigger
	...
	for (;;) {
		...
		for (j = 0; j < 8; j += E_BITS) {
			v ^= monPro(&M_[e >> (8 - E_BITS)], &t[v], &t[v ^ 1], modulus, Mc, Bc);
			count -= E_BITS;
			if (count == 0)
...
// repeat this step
	...
	NPRINT("exponentiation result: ", x_, rsa_get_len());
	return 0;
}
...
static uint16_t
    __attribute__((noinline)) rsaExpMod_montgomery_eblind(rsa_long_num t[2],
							  rsa_exp_num * exp, rsa_num * modulus)
{
	uint16_t count;
...
static uint8_t rsaGetKeyModulus(rsa_num * modulus, rsa_num * Bc, uint16_t size, uint8_t key)
{
	if (size != get_rsa_key_part(modulus, key)) {
		DPRINT("ERROR, unable to get (p/q) part of key\n");
		return 1;
	}
	if (!(modulus->value[0] & 1)) {
		DPRINT("ERROR, rsa prime (p/q) not odd (%02x)\n", modulus->value[0]);
		return 1;
	}
#ifndef USE_P_Q_INV
...
//#warning, fixed public exponent
	if (rsaExpMod_montgomery_init(t, TMP3, &Mc, M2, KEY_RSA_q))
		return Re_Q_GET_FAIL_1;
//                   message,exponent,modulus,Mc,Bc, public exponent (2^16+1)
...
//#warning, fixed public exponent
//                   mesasge,exponent,modulus,Mc,Bc, public exponent (2^16+1)
...
//    do not check exponentiation here (public exponent set to 0)
...
uint8_t rsa_crt_components(rsa_num pq[], rsa_num crt_comp[], uint16_t size)
{
// public exponent
//#warning, fixed public exponent
	memset(&crt_comp[0], 0, RSA_BYTES);

	crt_comp[0].value[0] = 1;
	crt_comp[0].value[2] = 1;
	...
	pq[0].value[0] &= 0xfe;
	pq[1].value[0] &= 0xfe;
	if (rsa_inv_mod(&crt_comp[1], &crt_comp[0], &pq[0]))
		return 1;
	if (rsa_inv_mod(&crt_comp[2], &crt_comp[0], &pq[1]))
		return 1;
	// add 1 back
	pq[0].value[0] |= 1;
	pq[1].value[0] |= 1;
	if (rsa_inv_mod(&crt_comp[3], &pq[1], &pq[0]))
		return 1;
#ifdef USE_P_Q_INV
	bn_set_bitlen(size * 8);

	rsa_inv_mod_N((rsa_half_num *) & crt_comp[4], &pq[0]);
	rsa_inv_mod_N((rsa_half_num *) & crt_comp[6], &pq[1]);

	barrett_constant(&crt_comp[5], &pq[0]);
	barrett_constant(&crt_comp[7], &pq[1]);
#endif
	return 0;
}
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// from epass2003 import we have only private exponent and modulus, for
// this allow us factorize the modulus and generate CRT components (up to 1024 bits)
//
//
typedef struct rsa_var_num {
	uint8_t value[1];
} rsa_var_num;

// 2048 OK, 
static void l_gcd(rsa_num * p, rsa_num * modulus)
{
	uint16_t len = rsa_get_len();

	rsa_var_num *v = alloca(len);
	rsa_var_num *u = alloca(len);
	rsa_var_num *tmp;

	memcpy(v, p, len);
	memcpy(u, modulus, len);	// u is odd ..
	for (;;) {
		// make "v" odd
		while ((v->value[0] & 1) == 0)
			bn_shiftr(v);
...
// modulus len is 2x len of p  - this function is used
// to divide modulus by p (and return q in "modulus"
// size rsa_get_len()
// 2048 OK
static void l_bn_div_modulus(void *modulus, void *p)
{
	uint16_t len = rsa_get_len();
	uint16_t i;
	uint8_t *d = alloca(len);
	uint8_t *r = alloca(len);

	// we assume that P has the highest bit set to 1
	// same for modulus ..
	NPRINT("m=", modulus, len);
	NPRINT("p=", p, len / 2);

	memset(d, 0, len);
	memset(r, 0, len);
	memcpy(d + len / 2, p, len / 2);

	// not very effective .. but working
	for (i = 1 + len * 4; i; i--) {
		bn_shiftl(r);
		if (bn_cmpGE(modulus, d)) {
			bn_sub(modulus, modulus, d);
			r[0] |= 1;
		}
		bn_shiftr(d);
	}
	NPRINT("q=", r, rsa_get_len());
	memcpy(modulus, r, rsa_get_len());
}

// Inspired by https://github.com/ius/rsatool

// size is "bit size" .. 
uint8_t rsa_factorize(uint8_t * p_exp, uint8_t * modulus, rsa_num pq[], uint16_t size)
{
	rsa_exp_num exponent;
	rsa_long_num tmpx;
	rsa_half_num Mc;
	rsa_long_num t[2];
	rsa_long_num *tmp = &tmpx;
	rsa_num *n = (rsa_num *) modulus;
	rsa_num *a = &(tmp->L);
	rsa_num *Bc = &(tmp->H);
	rsa_num c1;
	uint8_t ret, test;
	...
	DPRINT("factorization\n");

	// maximal multiplication size is (for now) 1024 ..
	if (size > 1024)
		return 1;

	DPRINT("exponent = ");
	NPRINT("t=", p_exp, size / 8);
	rsa_set_len(size / 8);

	// clear whole exponent 1st
	memset(&exponent, 0, sizeof(rsa_exp_num));
	// totient from private and public exponent
	// exponent  = p_exp * 0x10001 - 1
	//   0x000001 * p_exp
	memcpy(&exponent.value[0], p_exp, size / 8);
	// + 0x10000 * p_exp
	bn_add(&exponent.value[2], p_exp);
	// public exponent is always odd, zeroize bit 0 = subtract 1
	exponent.value[0] &= 0xfe;
	DPRINT("Totient = ");
	NPRINT("t=", &exponent.value, rsa_get_len());

	// (totient) exponent = e * (2 pow (d)), make "e" odd, calculate "d"
	do {
		d++;
		bn_shift_R_v_c(&exponent.value[0], 128 + 8, 0);
	}
	while ((exponent.value[0] & 1) == 0);

	NPRINT("quotient=", &exponent.value, rsa_get_len() + 8);
	DPRINT("s = %d\n", d);
...
// do not use exponent blinding here ..
	...
	for (test = 0; test < 50; test++) {
		...
		uint16_t i = 0;
		while (i++ < d) {
			memcpy(&c1, a, sizeof(rsa_num));
			if (bn_is_one(&c1)) {
//                              fprintf(stderr, "continue c1 = 1\n");
				continue;
			}
			n->value[0] &= 0xfe;
			ret = memcmp(n, &c1, sizeof(rsa_num));
			n->value[0] |= 1;
			if (ret == 0) {
//                              fprintf(stderr, "continue c1 = n - 1\n");
				continue;
			}
			//square..
			rsa_square(&t[1], a);
			...
			bn_mod_half(&t[1], n);
			memcpy(a, &t[1], rsa_get_len());
			NPRINT("squared c2=", a, rsa_get_len());
			if (!bn_is_one(a)) {
//                              fprintf(stderr, "continue c2 != 1\n");
				continue;
			}
			// c1 - 1
			NPRINT("c1=", &c1, rsa_get_len());
			memset(a, 0, rsa_get_len());
			a->value[0] = 1;
			rsa_sub(&c1, &c1, a);
			// gcd
			l_gcd(&c1, n);
			NPRINT("c1=", &c1, rsa_get_len());
			//
			l_bn_div_modulus(n, &c1);
			memset(p_exp, 0, size / 8);
			if (bn_cmpGE(n, &c1)) {
				memcpy(&pq[0], n, rsa_get_len());
				memcpy(&pq[1], &c1, rsa_get_len());
			} else {
				memcpy(&pq[0], &c1, rsa_get_len());
				memcpy(&pq[1], n, rsa_get_len());
			}
//                      fprintf(stderr, "fact. OK\n");
			//NPRINT("p=", &pq[0], rsa_get_len());
			//NPRINT("q=", &pq[1], rsa_get_len());
			return 0;
		}
	}
//      fprintf(stderr, "fact fail\n");
	return 1;
}
