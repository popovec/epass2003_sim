...
struct fs_data {
	// for record based EF size calculated from "type"
	uint16_t id;		// file id from tag 0x83
	uint16_t internal_size;	// size from tag 0x80 or 0x81 for transparent EF
	uint16_t fsize;		// from tag 0x80/0x81
	uint16_t uuid;		//
	uint16_t parent_uuid;	//
	uint8_t type[7];	// from tag 0x82 (size, max 6 bytes)
	uint8_t acl[8];
	uint8_t prop[3];	// from tag 0x85 (byte 0 = size)
...
struct fs_response {
	struct fs_data fs;
	uint16_t mem_offset;
	uint8_t current_record;	// for record based files
} __attribute__((__packed__));
...
static uint8_t get_lifecycle(void)
{
	uint8_t lc;
...
static void set_lifecycle(uint8_t lc)
{
	...
	temp = get_lifecycle();
	if (temp != local_lc)
		sec_device_write_block(&local_lc, offsetof(struct sec_device, lifecycle), 1);
}

static uint8_t is_DF(struct fs_response *entry)
{
	return (0xbf & entry->fs.type[1]) == 0x38;
}

// search file - parametric function based on "type" - most complicated routine to parse directory
// For lot of ISO7816 select types is here function to get proper file.
//
...
// file create need detect collision of ID under children and need maximal UUID
// this function return RET_SEARCH_END if no collision is detected (entry then contain correct uuid)
// or return on collision RET_SEARCH_OK is returned. Here parent and the immediate children of the parent DF
...
// id    - ID for test, if there is any collision
// data  - not relevant (NULL)
// type  - S_MAX
// RETURN - RET_SEARCH_END or RET_SEARCH_OK = collision
#define S_MAX     10
...
static uint8_t fs_search_file(struct fs_response *entry, uint16_t id, uint8_t * data, uint8_t type)
{
	uint16_t max_uuid = 0;
...
// get data for search from (selected) entry
	...
	while (0 == device_read_block(&response, response.mem_offset, sizeof(struct fs_data))) {
		DPRINT
		...
		if (response.fs.id == 0xffff) {
			...
			// not successful search, fill maximal uuid
			if (type == S_MAX) {
		...
		if (type == S_LIST_ALL) {
			...
			if (response.fs.parent_uuid == uuid)
				if (response.fs.id != 0x3f00)
					if (((response.fs.type[1] ^ ftype) & fmask) == 0)
						if (data_count < 127) {
							DPRINT
							    ("%s adding file %04x files count %d\n",
							     __FUNCTION__, response.fs.id,
							     data_count + 1);
...
// check if this ID is child of current DF
		if (response.fs.parent_uuid == uuid) {
			// DF ?
			if ((response.fs.type[1] & 0xbf) == 0x38) {
 ...
 fs_search_file_cont:
		...
		response.mem_offset += response.fs.internal_size;
	}
...
static int16_t __attribute__((noinline)) pin_position(uint8_t pin)
{
	...
	if (pin < 14) {
		position =
		    offsetof(struct sec_device, pins[1]) - offsetof(struct sec_device, pins[0]);
		position *= pin;
...
uint8_t fs_return_pin_info(uint8_t pin, struct iso7816_response *r)
{
	...
	if (sec_device_read_block(r->data, position + offsetof(struct pin, pin_retry), len))
...
static uint8_t compare_pin_puk(uint8_t pin, uint8_t * value, uint8_t puk)
{
	struct pin p;
	uint8_t ret = 0;
	int16_t position;
...
static uint8_t compare_pin(uint8_t pin, uint8_t * value)
{
	DPRINT("%s\n", __FUNCTION__);
...
// flag = 0 -> PIN/new PIN
// flag = 1 -> PUK/new PIN
// for 8 bytes only new PIN

uint8_t change_pin(uint8_t * message, uint8_t flag)
{
	struct pin p;
	uint8_t r;
	int16_t position;
	...
	if (message[1] == 0)
		...
		if (flag && (fs_get_access_condition() & SEC_ENABLE_UNBLOCK)
		    && (p.flags & (1 << 2)))
			r = 0;
...
static uint16_t check_security_pin_ac(uint8_t pin)
{
	uint16_t sec = 1;
	...
	if (pin == 0) {
		DPRINT("0 - ALWAYS ENABLED\n");
		return 0;
	};
	if (pin == 15) {
		DPRINT("15 - ALWAYS DISABLED\n");
		return 1;
	};
...
#else
	// allow only normal PIN for now, there is not enough info to implement grid pin and chalange response auth
	if (message[1] > 19) {
		if (message[21])
...
static uint8_t check_EF_security(uint8_t type)
{
	DPRINT("%s\n", __FUNCTION__);
...
static uint8_t check_DF_security(uint8_t type)
{
	DPRINT("%s\n", __FUNCTION__);
...
static uint16_t asn_get_uint_var(uint8_t * buffer, uint8_t size)
{
	if (size == 1)
		return *buffer;
...
static uint16_t asn_get_uint16(uint8_t * buffer)
{
	return buffer[0] << 8 | buffer[1];
}

static void fs_mkfs(uint8_t * message)
{
	DPRINT("%s\n", __FUNCTION__);
...
static uint8_t fs_get_fci(struct iso7816_response *r)
{
#ifndef EPASS2003
#error this code is only for EPASS2003!
#endif
	uint8_t *position;
	uint8_t len, i;
...
// sort response by tag value..
	...
	*(position++) = 2;
	*(position++) = fci_sel.fs.fsize >> 8;
	*(position++) = fci_sel.fs.fsize & 255;
	if (fci_sel.fs.type[0]) {
		len = fci_sel.fs.type[0];
		*(position++) = 0x82;
		*(position++) = len;
		i = 1;
		while (len--)
			*(position++) = fci_sel.fs.type[i++];
	}
	HPRINT("FCI:", &r->data[0], 40);
	*(position++) = 0x83;
	*(position++) = 2;
...
// name
	...
	if (fci_sel.fs.prop[0]) {
		*(position++) = 0x85;
		*(position++) = fci_sel.fs.prop[1];
		if (fci_sel.fs.prop[0] > 1)
			*(position++) = fci_sel.fs.prop[2];
	}
/*
  *(position++) = 0x86;
  *(position++) = 3;
...
/*
  ...
  *(position++) = get_lifecycle ();*/
	len = position - &r->data[0];
	r->data[1] = len - 2;
//      fprintf(stderr, "fci len =%d\n", len);
	RESP_READY(len);
}

static uint8_t
fs_return_selected(struct iso7816_response *r, uint16_t id, uint8_t * data, uint8_t type)
{
...
uint8_t fs_select_by_path_from_df(uint8_t * buffer, struct iso7816_response *r)
{
	DPRINT("%s\n", __FUNCTION__);
	return fs_return_selected(r, 0, buffer, S_PATH);
}

uint8_t fs_select_by_path_from_mf(uint8_t * buffer, struct iso7816_response *r)
{
	DPRINT("%s\n", __FUNCTION__);
...
static uint8_t fs_key_part(uint16_t * offset, uint8_t part)
{
	uint8_t tl[2];
	int16_t flen;

	DPRINT("%s part=%02x key file ID=%04x\n", __FUNCTION__, part, fci_sel.fs.id);
	if (part == 0xff)
...
// 0x11 for RSA, 0x13 EC private key  
...
// any file except DF can be used as key file
	if ((fci_sel.fs.type[1] & 0xbf) == 0x38)
		return C_KEYp_WRONG;	//this file is not designed to hold key
#endif
	flen = fci_sel.fs.fsize;
	DPRINT("%s file length = %d\n", __FUNCTION__, flen);
...
uint8_t fs_key_change_type(void)
{
	...
	if (check_EF_security(SEC_UPDATE) && check_EF_security(SEC_GENERATE))
		return S0x6982;	//security status not satisfied
	if (fci_sel.fs.type[1] == 0x23)
		return S_RET_OK;
	if (fci_sel.fs.type[1] != 0x22)
		return S0x6985;	// condition of use not satisfied
	fci_sel.fs.type[1] = 0x23;
	if (device_write_block(&fci_sel.fs, fci_sel.mem_offset, sizeof(struct fs_data)))
		return S0x6581;	//memory fail
...
*/
	...
	if (size_test + K_SIZE > fci_sel.fs.fsize)
...
static uint8_t fs_transparent_file(void)
{
	// do not test shareable file flag
	if ((fci_sel.fs.type[1] & 0xbf) == 1)
		return 1;
	DPRINT("%s fail, file type %02x\n", __FUNCTION__, fci_sel.fs.type[1]);
	return 0;
}

// functions for record based files
// internal structure:
// uint8_t xx - record number
// uint16_t size - record size
// record data...

struct search_record {
	uint16_t fs_position;
	uint16_t offset;
	uint8_t record;
	uint16_t length;
};
// simple TLV tag, len
// tag 0 and 0xff is not alloved (iso7816-4 5.2.1)

static uint8_t fs_get_record_(struct search_record *s)
{

	uint8_t tl[3];
	uint16_t size;
	uint16_t offset = s->offset + s->length;

	s->offset = offset;

	// read record number, length
	if (fci_sel.fs.internal_size - offset > 3)
		size = 3;
	else
		size = fci_sel.fs.internal_size - offset;
	if (size < 3)
		return S0x6a88;	// reference data not found
	if (1 == device_read_block(tl, offset + s->fs_position, 3))
		return S0x6581;	// Memory failure
	s->record = tl[0];
	s->length = 3 + (tl[1] << 8) + tl[2];
	return S_RET_OK;
}

uint8_t fs_read_record(uint8_t record, struct iso7816_response *r)
{
	struct search_record s;
	uint8_t ret;
	uint16_t len;

	if (record == 0)
		record = fci_sel.current_record;
// record 0xff is reserved (iso7816-4 7.3.2)
	if (record == 0xff)
		return S0x6a83;	// record not found

// TODO file type check, ACL check

// position of data part of file in filesystem
	s.fs_position = fci_sel.mem_offset + sizeof(struct fs_data) + fci_sel.fs.name_size;
	s.offset = 0;
	s.length = 0;
	for (;;) {
		ret = fs_get_record_(&s);
		if (ret != S_RET_OK)
			return ret;
		// free space in file ? 
		if (s.record == 0xff)
	...
	len = s.length - 3;
	// TODO record length is limited here
	if (len > 250)
		len = 250;
	if (1 == device_read_block(r->data, s.offset + s.fs_position + 3, len))
		return S0x6581;	// Memory failure
	RESP_READY(len);
}

uint8_t fs_write_record(uint8_t record, uint8_t type, struct iso7816_response *r)
{
	struct search_record s;
	uint8_t ret;
//
	if (type == T_RECORD_APPEND)
		record = 1;
//
	if (record == 0)
		record = fci_sel.current_record;

// limit record size (3 bytes for record number and size)
	if (r->Nc > 253)
		return S0x6a84;	// Not enough memory space in the file
// record 0xff is reserved (iso7816-4 7.3.2)
	if (record == 0xff)
		return S0x6a83;	// record not found
// TODO file type check, ACL check
...
// position of data part of file in filesystem
	...
	for (;;) {
		...
		if (ret != S_RET_OK)
			return ret;
		if (s.record == record) {
			// there is already record in this position, and not in logical erased state
			// iso7816-4 7.3.4
			return S0x6981;	//command incompatible with file structure
		}
		// free space in file ? 
		if (s.record == 0xff)
			break;
	...
	if (s.offset + 3 + r->Nc > fci_sel.fs.internal_size)
		return S0x6a84;	// Not enough memory space in the file
	r->input[2] = record;
	r->input[3] = 0;
	r->input[4] = r->Nc;
	if (1 == device_write_block(r->input + 2, s.offset + s.fs_position, r->Nc + 3))
		return S0x6581;	// Memory failure
	return S_RET_OK;
}
...
uint8_t fs_read_binary(uint16_t offset, struct iso7816_response *r)
{
	...
	if (fci_sel.fs.type[1] != 0x12)	// Epass2003 RSA public key 
		if (fci_sel.fs.type[1] != 0x14)	// Epass2003 EC public key
...
uint8_t fs_update_binary(uint8_t * buffer, uint16_t offset)
{
	...
	if (fci_sel.fs.type[1] != 0x12)	// Epass2003 public key  (RSA)
		if (fci_sel.fs.type[1] != 0x14)	// Epass2003 public key (EC)
			if (!fs_transparent_file())
				return S0x6985;	// condition of use not satisfied
...
uint8_t fs_erase_binary(uint16_t offset)
{
	...
	if (offset > fci_sel.fs.fsize)
		return S0x6b00;	//outside EF
	size = fci_sel.fs.fsize - offset;
	offset += fci_sel.mem_offset;
	offset += sizeof(struct fs_data);
	offset += fci_sel.fs.name_size;
...
// DF subtree delete is not very effective, because small ram, there is no way to do
// recursion or mark a path in subtree into RAM.
// return 1 on mem error
// delete this ef_df
static uint8_t fs_delete_this_ef_df(struct fs_response *desc)
{
	DPRINT("%s DELETING ID %04x\n", __FUNCTION__, desc->fs.id);
	// mark file as deleted
	desc->fs.active = 0;
	if (device_write_block(&desc->fs, desc->mem_offset, sizeof(struct fs_data)))
...
// return 1 on mem error
// delete all EF in DF
static uint8_t fs_delete_all_ef(struct fs_response *df)
{
...
static uint8_t fs_delete_df_subtree(struct fs_response *df)
{
	struct fs_response next;
...
uint8_t fs_delete_file(void)
{
	...
	// special handling for MF (card erase)
	if (fci_sel.fs.id == 0x3f00)
		return fs_erase_card(NULL);
// get parent
...
// data bytes in file (excluding struct. info)
		case 0x80:
			...
			flag |= 1;
			fs.fsize = asn_get_uint_var(buffer, dlen);
// limit filesize
			if (fs.fsize > 32767)
				return S0x6984;	//invalid data
			break;
...
// epass2003 - ignore MF creation
			if (fs.id == 0x3f00)	// MF
				return S_RET_OK;	//invalid data
...
// filename
	...
	fs.internal_size = fs.fsize;
	if (fs.type[1] == 4) {
		uint16_t records = 1;
		if (fs.type[0] == 5) {
			records = fs.type[5] + 1;
			fs.internal_size = ((fs.type[3] << 8) + fs.type[4]) * records;
		} else if (fs.type[0] == 6) {
			records = (fs.type[5] << 8) + fs.type[6] + 1;
			fs.internal_size = ((fs.type[3] << 8) + fs.type[4]) * records;
		} else if (fs.type[0] == 4)
			fs.internal_size = ((fs.type[3] << 8) + fs.type[4]);
		else if (fs.type[0] == 3)
			fs.internal_size = fs.type[3];
		else
			// default (workaround)
			fs.internal_size = 128;
	}
// EC private key
	if (fs.type[1] == 0x13) {
		fs.tag_80_81 = 0;
		// real file size
		fs.internal_size = ((fs.prop[1] << 8) + fs.prop[2]);
		fs.fsize = fs.internal_size;
	}
// EC public key
	if (fs.type[1] == 0x14) {
		fs.tag_80_81 = 0;
		// real file size
		// format  0x58, len, X[], 0x59, len, Y[]
		fs.internal_size = ((fs.prop[1] << 8) + fs.prop[2]) / 2 + 4;
		fs.fsize = fs.internal_size;
	}
// RSA public key
	if (fs.type[1] == 0x12) {
		fs.tag_80_81 = 0;
		// real file size (exact modulus length)
		fs.internal_size = ((fs.prop[1] << 8) + fs.prop[2]) / 8;
		// temp size, (this file is erased and then recreated as type 0x01)
		fs.fsize = fs.internal_size;
	}
// RSA private key
	if (fs.type[1] == 0x11) {
...
*/
	...
	// search new UUID (and collision test)
	...
	if (0 !=
	    device_read_block(&flag,
			      ...
			      fs.name_size + fs.internal_size + sizeof(struct fs_data), 1))
